...
pub enum ExitKind {
    ...
    /// device is in ExitToUserspace mode. Userspace should read the pending
    /// request (PID + length) via `Vm::random_request()`, stage the reply
    /// bytes with `Vm::set_random_bytes()`, and run again.
    VmcallGetRandom,
