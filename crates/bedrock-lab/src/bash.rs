// SPDX-License-Identifier: GPL-2.0

//! I/O channel actions: bash command injection and workload introspection.
//!
//! The hypervisor exposes a deterministic guest↔host I/O channel; in
//! combination with the in-guest `bedrock-io.ko` module it lets the lab
//! dispatch shell commands and query the guest's workload listing. This
//! module wraps the wire protocol so callers can think in terms of "run
//! this command" or "list workloads" rather than byte layouts.

/// Where a [`Branch::bash`](crate::Branch::bash) command runs.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum BashTarget {
    /// Run on the guest host (outside any container).
    Host,
    /// Run inside the named container.
    Container(String),
}

impl BashTarget {
    /// Construct a [`BashTarget::Host`].
    pub fn host() -> Self {
        Self::Host
    }

    /// Construct a [`BashTarget::Container`] without typing `.into()`.
    pub fn container(name: impl Into<String>) -> Self {
        Self::Container(name.into())
    }
}

/// Result of a [`Branch::bash`](crate::Branch::bash) call.
///
/// Exec actions stream stdout/stderr into the guest journal via `systemd-cat`,
/// so the host-bound response carries only status and exit code — observe the
/// command's output through serial events on the [`EventSink`](crate::EventSink)
/// (the journal lines come out as `[<tag>] | …`), not through this struct.
#[derive(Debug, Clone)]
pub struct BashOutput {
    /// Action-level status from the guest module. `0` means the action was
    /// dispatched successfully; non-zero typically means the requested
    /// container/binary couldn't be found.
    pub status: i32,
    /// Exit code of the bash command itself.
    pub exit_code: i32,
}

impl BashOutput {
    /// Returns `true` iff the action dispatched and bash exited with 0.
    pub fn success(&self) -> bool {
        self.status == 0 && self.exit_code == 0
    }
}

/// One entry in a workload listing — a container and an executable "driver"
/// path inside it that can be invoked via `Branch::bash` with
/// `BashTarget::Container`.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct WorkloadDriver {
    pub container: String,
    pub driver: String,
}

/// Result of a [`Branch::workload_details`](crate::Branch::workload_details)
/// query. Splits the guest's listing into two views of the same data:
///
/// - `containers`: every running container in alphabetical order, even
///   those without any `/opt/bedrock/drivers/` entry. Useful for
///   targeting host-level actions (e.g. fault injection) at containers
///   that don't ship drivers themselves.
/// - `drivers`: container/driver pairs for every executable found under
///   `/opt/bedrock/drivers/` inside each container.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct WorkloadDetails {
    pub containers: Vec<String>,
    pub drivers: Vec<WorkloadDriver>,
}

// --- Wire protocol ---
//
// Mirrors the `bedrock-io.ko` ABI. Keep in sync with the kernel module's
// IO_REQUEST_MAGIC / IO_RESPONSE_MAGIC constants and action IDs.

pub(crate) const IO_REQUEST_MAGIC: u32 = 0xB10C1010;
pub(crate) const IO_RESPONSE_MAGIC: u32 = 0x1010B10C;

const ACTION_GET_WORKLOAD_DETAILS: u32 = 0;
const ACTION_EXEC_BASH: u32 = 1;
const ACTION_EXEC_HOST_BASH: u32 = 2;

const REQUEST_HEADER_LEN: usize = 16;
const RESPONSE_HEADER_LEN: usize = 24;

/// Encode a bash request targeting the host or a named container.
/// `request_id` is the host's opaque tag, echoed by the kernel module
/// in the response so the lab can correlate this submission with its
/// specific reply even when other workers complete out of order.
pub(crate) fn encode_bash_request(target: BashTarget, request_id: u32, cmd: &str) -> Vec<u8> {
    let (action_id, payload_len) = match &target {
        BashTarget::Host => (ACTION_EXEC_HOST_BASH, cmd.len() + 1),
        BashTarget::Container(name) => (ACTION_EXEC_BASH, name.len() + 1 + cmd.len() + 1),
    };
    let mut bytes = Vec::with_capacity(REQUEST_HEADER_LEN + payload_len);
    bytes.extend_from_slice(&IO_REQUEST_MAGIC.to_le_bytes());
    bytes.extend_from_slice(&action_id.to_le_bytes());
    bytes.extend_from_slice(&request_id.to_le_bytes());
    bytes.extend_from_slice(&(payload_len as u32).to_le_bytes());
    if let BashTarget::Container(name) = target {
        bytes.extend_from_slice(name.as_bytes());
        bytes.push(0);
    }
    bytes.extend_from_slice(cmd.as_bytes());
    bytes.push(0);
    bytes
}

/// Encode a workload-details request (empty payload). See
/// `encode_bash_request` for the `request_id` contract.
pub(crate) fn encode_workload_details_request(request_id: u32) -> Vec<u8> {
    let mut bytes = Vec::with_capacity(REQUEST_HEADER_LEN);
    bytes.extend_from_slice(&IO_REQUEST_MAGIC.to_le_bytes());
    bytes.extend_from_slice(&ACTION_GET_WORKLOAD_DETAILS.to_le_bytes());
    bytes.extend_from_slice(&request_id.to_le_bytes());
    bytes.extend_from_slice(&0u32.to_le_bytes());
    bytes
}

/// Decoded I/O channel response, dispatched by action ID.
#[derive(Debug, Clone)]
pub enum ActionResponse {
    Bash(BashOutput),
    WorkloadDetails(WorkloadDetails),
}

struct Envelope<'a> {
    action_id: u32,
    request_id: u32,
    status: i32,
    exit_code: i32,
    data: &'a [u8],
}

fn decode_envelope(bytes: &[u8]) -> Result<Envelope<'_>, String> {
    if bytes.len() < RESPONSE_HEADER_LEN {
        return Err(format!("response too short: {} bytes", bytes.len()));
    }
    let magic = u32::from_le_bytes([bytes[0], bytes[1], bytes[2], bytes[3]]);
    if magic != IO_RESPONSE_MAGIC {
        return Err(format!("bad response magic {:#x}", magic));
    }
    let action_id = u32::from_le_bytes([bytes[4], bytes[5], bytes[6], bytes[7]]);
    let request_id = u32::from_le_bytes([bytes[8], bytes[9], bytes[10], bytes[11]]);
    let status = i32::from_le_bytes([bytes[12], bytes[13], bytes[14], bytes[15]]);
    let exit_code = i32::from_le_bytes([bytes[16], bytes[17], bytes[18], bytes[19]]);
    let data_len = u32::from_le_bytes([bytes[20], bytes[21], bytes[22], bytes[23]]) as usize;
    let data_end = RESPONSE_HEADER_LEN + data_len;
    if data_end > bytes.len() {
        return Err(format!(
            "response data overruns: {} > {}",
            data_end,
            bytes.len()
        ));
    }
    Ok(Envelope {
        action_id,
        request_id,
        status,
        exit_code,
        data: &bytes[RESPONSE_HEADER_LEN..data_end],
    })
}

fn parse_workload_listing(data: &[u8]) -> WorkloadDetails {
    let text = String::from_utf8_lossy(data);
    let mut details = WorkloadDetails::default();
    for line in text.lines() {
        let Some((container, driver)) = line.split_once('\t') else {
            continue;
        };
        // The guest emits each container once as a `<name>\t` header
        // followed by zero or more `<name>\t<driver>` lines, sorted
        // alphabetically. Preserve that order in `containers` by
        // pushing on the header line only; deduplicate defensively in
        // case the protocol ever changes.
        if driver.is_empty() {
            if details.containers.last().map(String::as_str) != Some(container) {
                details.containers.push(container.to_string());
            }
        } else {
            details.drivers.push(WorkloadDriver {
                container: container.to_string(),
                driver: driver.to_string(),
            });
        }
    }
    details
}

/// Decode an I/O channel response. Returns `(request_id, response)` so
/// the caller can correlate to a specific outstanding submission. The
/// `request_id` is whatever the host put in the matching request — the
/// kernel module never interprets it.
pub(crate) fn decode_response(bytes: &[u8]) -> Result<(u32, ActionResponse), String> {
    let env = decode_envelope(bytes)?;
    let response = match env.action_id {
        ACTION_GET_WORKLOAD_DETAILS => {
            ActionResponse::WorkloadDetails(parse_workload_listing(env.data))
        }
        ACTION_EXEC_BASH | ACTION_EXEC_HOST_BASH => ActionResponse::Bash(BashOutput {
            status: env.status,
            exit_code: env.exit_code,
        }),
        other => return Err(format!("unknown action_id: {other}")),
    };
    Ok((env.request_id, response))
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn encode_host_layout() {
        let bytes = encode_bash_request(BashTarget::Host, 0xabcd_1234, "echo hi");
        assert_eq!(&bytes[0..4], &IO_REQUEST_MAGIC.to_le_bytes());
        assert_eq!(&bytes[4..8], &ACTION_EXEC_HOST_BASH.to_le_bytes());
        assert_eq!(&bytes[8..12], &0xabcd_1234u32.to_le_bytes());
        // payload len = "echo hi\0" = 8
        assert_eq!(&bytes[12..16], &8u32.to_le_bytes());
        assert_eq!(&bytes[16..24], b"echo hi\0");
    }

    #[test]
    fn encode_container_layout() {
        let bytes = encode_bash_request(BashTarget::container("ctr"), 7, "ls");
        assert_eq!(&bytes[4..8], &ACTION_EXEC_BASH.to_le_bytes());
        assert_eq!(&bytes[8..12], &7u32.to_le_bytes());
        // "ctr\0ls\0" = 7
        assert_eq!(&bytes[12..16], &7u32.to_le_bytes());
        assert_eq!(&bytes[16..23], b"ctr\0ls\0");
    }

    #[test]
    fn encode_workload_details_layout() {
        let bytes = encode_workload_details_request(42);
        assert_eq!(bytes.len(), REQUEST_HEADER_LEN);
        assert_eq!(&bytes[0..4], &IO_REQUEST_MAGIC.to_le_bytes());
        assert_eq!(&bytes[4..8], &ACTION_GET_WORKLOAD_DETAILS.to_le_bytes());
        assert_eq!(&bytes[8..12], &42u32.to_le_bytes());
        assert_eq!(&bytes[12..16], &0u32.to_le_bytes());
    }

    fn make_response(
        action_id: u32,
        request_id: u32,
        status: i32,
        exit_code: i32,
        data: &[u8],
    ) -> Vec<u8> {
        let mut resp = Vec::new();
        resp.extend_from_slice(&IO_RESPONSE_MAGIC.to_le_bytes());
        resp.extend_from_slice(&action_id.to_le_bytes());
        resp.extend_from_slice(&request_id.to_le_bytes());
        resp.extend_from_slice(&status.to_le_bytes());
        resp.extend_from_slice(&exit_code.to_le_bytes());
        resp.extend_from_slice(&(data.len() as u32).to_le_bytes());
        resp.extend_from_slice(data);
        resp
    }

    #[test]
    fn decode_bash_response_roundtrip() {
        let resp = make_response(ACTION_EXEC_HOST_BASH, 99, 0, 42, b"");
        let (rid, parsed) = decode_response(&resp).unwrap();
        assert_eq!(rid, 99);
        match parsed {
            ActionResponse::Bash(out) => {
                assert_eq!(out.status, 0);
                assert_eq!(out.exit_code, 42);
            }
            other => panic!("expected Bash, got {other:?}"),
        }
    }

    #[test]
    fn decode_container_bash_response_uses_bash_variant() {
        let resp = make_response(ACTION_EXEC_BASH, 1, 0, 0, b"");
        let (rid, parsed) = decode_response(&resp).unwrap();
        assert_eq!(rid, 1);
        assert!(matches!(parsed, ActionResponse::Bash(_)));
    }

    #[test]
    fn decode_workload_details_parses_lines() {
        let body = "header1\t\nworker-1\t/drivers/a\nworker-2\t/drivers/b\nempty\t\n";
        let resp = make_response(ACTION_GET_WORKLOAD_DETAILS, 5, 0, 0, body.as_bytes());
        let (rid, parsed) = decode_response(&resp).unwrap();
        assert_eq!(rid, 5);
        match parsed {
            ActionResponse::WorkloadDetails(details) => {
                assert_eq!(
                    details.containers,
                    vec!["header1".to_string(), "empty".to_string()]
                );
                assert_eq!(
                    details.drivers,
                    vec![
                        WorkloadDriver {
                            container: "worker-1".into(),
                            driver: "/drivers/a".into()
                        },
                        WorkloadDriver {
                            container: "worker-2".into(),
                            driver: "/drivers/b".into()
                        },
                    ]
                );
            }
            other => panic!("expected WorkloadDetails, got {other:?}"),
        }
    }

    #[test]
    fn decode_rejects_bad_magic() {
        let mut resp = make_response(0, 0, 0, 0, b"");
        resp[0] = 0; // corrupt magic
        assert!(decode_response(&resp).is_err());
    }

    #[test]
    fn decode_rejects_unknown_action() {
        let resp = make_response(99, 0, 0, 0, b"");
        assert!(decode_response(&resp).is_err());
    }
}
