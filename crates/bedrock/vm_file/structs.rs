...
pub(crate) const BEDROCK_VM_GET_RANDOM_REQUEST: u32 =
    _IOR::<BedrockRandomRequest>(BEDROCK_IOC_MAGIC, 15);
...
pub(crate) const BEDROCK_VM_SET_RANDOM_BYTES: u32 =
    _IOW::<BedrockRandomBytes>(BEDROCK_IOC_MAGIC, 16);
...
/// Pending `HYPERCALL_GET_RANDOM` request returned to userspace, so it can
/// size the reply and attribute the request to a process.
#[repr(C)]
