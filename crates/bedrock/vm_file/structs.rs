...
use kernel::bindings;
use kernel::ioctl::{_IO, _IOR, _IOW};
...
pub const BEDROCK_IOC_MAGIC: u32 = b'B' as u32;

/// Ioctl number for GET_REGS command - read all VM registers.
pub const BEDROCK_VM_GET_REGS: u32 = _IOR::<BedrockRegs>(BEDROCK_IOC_MAGIC, 1);

/// Ioctl number for SET_REGS command - write all VM registers.
pub const BEDROCK_VM_SET_REGS: u32 = _IOW::<BedrockRegs>(BEDROCK_IOC_MAGIC, 2);

/// Ioctl number for RUN command - run the VM until exit.
pub const BEDROCK_VM_RUN: u32 = _IOR::<BedrockVmExit>(BEDROCK_IOC_MAGIC, 3);

/// Ioctl number for SET_INPUT command - set serial input buffer.
pub const BEDROCK_VM_SET_INPUT: u32 = _IOW::<BedrockSerialInput>(BEDROCK_IOC_MAGIC, 4);

/// Ioctl number for SET_RDRAND_CONFIG command - configure RDRAND emulation.
pub const BEDROCK_VM_SET_RDRAND_CONFIG: u32 = _IOW::<BedrockRdrandConfig>(BEDROCK_IOC_MAGIC, 5);

/// Ioctl number for SET_RDRAND_VALUE command - set pending RDRAND value.
pub const BEDROCK_VM_SET_RDRAND_VALUE: u32 = _IOW::<u64>(BEDROCK_IOC_MAGIC, 6);
...
pub const BEDROCK_VM_SET_LOG_CONFIG: u32 = _IOW::<BedrockLogConfig>(BEDROCK_IOC_MAGIC, 7);

/// Ioctl number for SET_SINGLE_STEP command - configure MTF single-stepping.
pub const BEDROCK_VM_SET_SINGLE_STEP: u32 = _IOW::<BedrockSingleStepConfig>(BEDROCK_IOC_MAGIC, 8);

/// Ioctl number for GET_EXIT_STATS command - retrieve exit handler performance statistics.
pub const BEDROCK_VM_GET_EXIT_STATS: u32 = _IOR::<BedrockExitStats>(BEDROCK_IOC_MAGIC, 9);

/// Ioctl number for SET_STOP_TSC command - stop VM when TSC reaches this value.
pub const BEDROCK_VM_SET_STOP_TSC: u32 = _IOW::<u64>(BEDROCK_IOC_MAGIC, 10);

/// Ioctl number for GET_VM_ID command - get the VM's unique identifier.
pub const BEDROCK_VM_GET_VM_ID: u32 = _IOR::<u64>(BEDROCK_IOC_MAGIC, 11);
...
pub const BEDROCK_VM_GET_FEEDBACK_BUFFER_INFO: u32 =
...
#[repr(C)]
pub struct BedrockFeedbackBufferInfoRequest {
...
#[repr(C)]
pub struct BedrockFeedbackBufferInfo {
...
#[repr(C)]
pub struct BedrockRdrandConfig {
...
#[repr(C)]
pub struct BedrockSingleStepConfig {
...
pub struct BedrockExitStatEntry {
...
pub struct SyncFileOps(pub bindings::file_operations);
...
impl SyncFileOps {
    ...
    pub const unsafe fn zeroed() -> bindings::file_operations {
