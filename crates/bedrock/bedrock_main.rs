...
fn handle_create_forked_vm(parent_vm_id: u64) -> Result<isize> {
    ...
    let fork_result = {
        ...
        // The fork gets its own perf_event with the same seed period as the
        // root path. update_mtf_state will realign it as soon as the fork
        // sees a forced-exit target.
        let instruction_counter = LinuxInstructionCounter::new(factory::PMI_SEED_PERIOD)
