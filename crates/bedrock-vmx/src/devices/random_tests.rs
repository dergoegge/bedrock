...
#[test]
fn needs_userspace_exit_only_in_source_mode_without_reply() {
    let mut s = RandomState::default();
    s.configure(RdrandMode::SeededRng, 7);
    assert!(!s.needs_userspace_exit(), "seeded never exits to userspace");

    s.configure(RdrandMode::ExitToUserspace, 0);
    assert!(s.needs_userspace_exit(), "source mode with no staged reply");

    s.begin_request(0xdead_0000, 32, 1234);
    assert!(s.awaiting);
    assert!(s.needs_userspace_exit(), "still awaiting before staging");

    s.stage_reply(&[0xAB; 32]);
    assert!(s.reply_valid);
    assert_eq!(s.reply_len, 32);
    assert!(!s.needs_userspace_exit(), "reply staged → no further exit");
