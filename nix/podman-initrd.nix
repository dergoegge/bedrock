...
let
  ...
  workloadMonitor = pkgs.pkgsStatic.rustPlatform.buildRustPackage {
    ...
    cargoLock.lockFile = ../Cargo.lock;
    cargoBuildFlags = [ "-p" "bedrock-workload-monitor" ];
    doCheck = false;
    meta.mainProgram = "bedrock-workload-monitor";
  };
  ...
  initScript = pkgs.writeScript "init" ''
    ...
    # Shared assertion sink: an append-only JSONL file, one assertion per line.
    # The host-side workload monitor appends to it, and containers.conf
    # bind-mounts it into every container so workload code can append too. Create
    # it as a regular file now — before the monitor starts and before
    ...
    # `[assertions] | …`. `-F` re-follows across truncation/recreation and
    # `-n +1` replays existing lines so nothing written before this starts is
    # missed.
