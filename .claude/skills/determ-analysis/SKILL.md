...
│   ├── exit-log.jsonl          # Deterministic exit data (JSONL)
│   ├── exit-log-nondeterm.jsonl  # Non-deterministic exits (auto-generated by bedrock-cli)
...
`bedrock-cli` produces two JSONL files when `--log-jsonl` is specified:
- `exit-log.jsonl` — deterministic exits (exits where `flags & 1 != 0`)
- `exit-log-nondeterm.jsonl` — non-deterministic exits (auto-generated alongside exit-log.jsonl by stripping the `.jsonl` suffix and appending `-nondeterm.jsonl`)

Both files have the same entry format. The non-determ log is only populated when exit logging is enabled (`--enable-log`, `--log-checkpoints`, or `--log-at-shutdown`).
...
- `flags` (u32) - Bit 0 = deterministic exit
...
The determinism test binary passes one of these to bedrock-cli:
- `--log-at-shutdown` (default) — single entry at VM shutdown
- `--log-checkpoints INTERVAL` — periodic snapshots at TSC intervals
- `--enable-log` (via `--log-all-exits`) — every deterministic exit (large logs)

Additional modifiers:
- `--no-memory-hash` — skip memory hashing (memory_hash will be 0)
- `--log-after-tsc TSC` — only start logging after this TSC value
- `--single-step START-END` — MTF single-stepping in a TSC range
...
Note the key parameters: `runs`, `parallel`, `stop_at_tsc`/`stop_at_vt`, `checkpoint_interval`, `log_all_exits`, `parent_id`, `no_memory_hash`, `intercept_pf`.
...
# Exit log sizes
ssh $REMOTE_HOST 'wc -l ~/workspace/determ-tests/<dir>/run-NNNNNN/exit-log.jsonl ~/workspace/determ-tests/<dir>/run-001/exit-log.jsonl'
ssh $REMOTE_HOST 'wc -l ~/workspace/determ-tests/<dir>/run-NNNNNN/exit-log-nondeterm.jsonl ~/workspace/determ-tests/<dir>/run-001/exit-log-nondeterm.jsonl'
```
